...
export class AppData extends Model<IAppState>{
	...
	public setProducts(items: IProduct[]) {//поменять в документации setProducts(items: IProduct[])
		this._products = items
		this.emitChanges('items:changed', this._products)//поменять в документации
	}
	public setPreview(items: IProduct) { //поменять в документации
		this._preview = items;
		this.emitChanges('preview:change', this._preview)//поменять в документации
	}

	public setContacts(email:string,phone:string){ //надо ли? как будто нет, потому что они не устанавливаются, а вводятся пользователем
	...
	public getTotal() { //добавлено
		if (this.isBasketHaveProductPriceless()) {
			return 'Бесценно'
		}
		return this._basket.itemsBasket.reduce((a, c) => a + (c.price as number), 0)
	}
	...
	private validateEmptyField(field: keyof IOrder, errorMessage: string): boolean { //добавить в документацию
	...
	private isOrderValidForm(){
		this.validateEmptyField('address', 'Необходимо указать адрес');
		this.validateEmptyField('payment', 'Необходимо указать тип оплаты');
		if (this.validateEmptyField('email', 'Необходимо указать электронную почту')) {
			this.validateFieldRegex('email', 'Некорректный формат электронной почты', regexPatterns[RegexEnum.Email])
		}
		if (this.validateEmptyField('phone', 'Необходимо указать номер телефона')) {
			this.validateFieldRegex('phone', 'Некорректный формат номера телефона', regexPatterns[RegexEnum.PhoneNumber])
		}
